...
public class FileDrop {
        ...
        if (supportsDnD()) {   // Make a drop listener
            dropListener = new java.awt.dnd.DropTargetListener() {
                public void dragEnter(java.awt.dnd.DropTargetDragEvent evt) {
                    Hotels.log("FileDrop: dragEnter event.");
                    ...
                    if (isDragOk(out, evt)) {
                        ...
                        if (c instanceof javax.swing.JComponent) {
                            ...
                            Hotels.log("FileDrop: normal border saved.");
                            jc.setBorder(dragBorder);
                            Hotels.log("FileDrop: drag border set.");
                        }   // end if: JComponent   

                        // Acknowledge that it's okay to enter
                        //evt.acceptDrag( java.awt.dnd.DnDConstants.ACTION_COPY_OR_MOVE );
                        evt.acceptDrag(java.awt.dnd.DnDConstants.ACTION_COPY);
                        Hotels.log("FileDrop: event accepted.");
                    }   // end if: drag ok
                    else {   // Reject the drag event
                        evt.rejectDrag();
                        Hotels.log("FileDrop: event rejected.");
                ...
                public void drop(java.awt.dnd.DropTargetDropEvent evt) {
                	Hotels.log("FileDrop: drop event.");
                    try {   // Get whatever was dropped
                        ...
                        if (tr.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) {
                            ...
                            Hotels.log("FileDrop: file list accepted.");
                            ...
                            Hotels.log("FileDrop: drop complete.");
                        }   // end if: file list
                        else // this section will check for a reader flavor.
                        {
                            ...
                            for (int zz = 0; zz < flavors.length; zz++) {
                                if (flavors[zz].isRepresentationClassReader()) {
                                    ...
                                    //evt.acceptDrop ( java.awt.dnd.DnDConstants.ACTION_COPY_OR_MOVE );
                                    evt.acceptDrop(java.awt.dnd.DnDConstants.ACTION_COPY);
                                    Hotels.log("FileDrop: reader accepted.");
                            ...
                            if (!handled) {
                            	Hotels.log("FileDrop: not a file list or reader - abort.");
                                evt.rejectDrop();
                            }
                    	...
                    	Hotels.log("FileDrop: IOException - abort:");
                        io.printStackTrace(out);
                        evt.rejectDrop();
                    }   // end catch IOException
                    catch (java.awt.datatransfer.UnsupportedFlavorException ufe) {
                    	Hotels.log("FileDrop: UnsupportedFlavorException - abort:");
                        ...
                        if (c instanceof javax.swing.JComponent) {
                            ...
                            Hotels.log("FileDrop: normal border restored.");
                        }   // end if: JComponent
                    }   // end finally
                }   // end drop

                public void dragExit(java.awt.dnd.DropTargetEvent evt) {
                	Hotels.log("FileDrop: dragExit event.");
                    // If it's a Swing component, reset its border
                    if (c instanceof javax.swing.JComponent) {
                        javax.swing.JComponent jc = (javax.swing.JComponent) c;
                        jc.setBorder(normalBorder);
                ...
                public void dropActionChanged(java.awt.dnd.DropTargetDragEvent evt) {
                	Hotels.log("FileDrop: dropActionChanged event.");
                    // Is this an acceptable drag event?
        	...
        	Hotels.log("FileDrop: Drag and drop is not supported with this JVM");
        ...
        try {
            ...
            while ((line = bReader.readLine()) != null) {
                try {
                	...
                	Hotels.log("Error with " + line + ": " + ex.getMessage());
                }
            }

            return (File[]) list.toArray(new File[list.size()]);
        } catch (IOException ex) {
        	Hotels.log("FileDrop: IOException");
        }
        return new File[0];
    }
    // END 2007-09-12 Nathan Blomquist -- Linux (KDE/Gnome) support added.
    ...
    private void makeDropTarget(final PrintStream out, final java.awt.Component c, boolean recursive) {
        ...
        try {
            ...
            e.printStackTrace();
            Hotels.log("FileDrop: Drop will not work due to previous error. Do you have another listener attached?");
        }   // end catch

        // Listen for hierarchy changes and remove the drop target when the parent gets cleared out.
        c.addHierarchyListener(new java.awt.event.HierarchyListener() {
            public void hierarchyChanged(java.awt.event.HierarchyEvent evt) {
            	Hotels.log("FileDrop: Hierarchy changed.");
                java.awt.Component parent = c.getParent();
                if (parent == null) {
                    c.setDropTarget(null);
                    Hotels.log("FileDrop: Drop target cleared from component.");
                }   // end if: null parent
                else {
                    new java.awt.dnd.DropTarget(c, dropListener);
                    Hotels.log("FileDrop: Drop target added to component.");
    ...
    private boolean isDragOk(final PrintStream out, final java.awt.dnd.DropTargetDragEvent evt) {
        ...
        if (out != null) {
            if (flavors.length == 0)
            	Hotels.log("FileDrop: no data flavors.");
            for (i = 0; i < flavors.length; i++)
            	Hotels.log(flavors[i].toString());
    ...
    public static boolean remove(PrintStream out, java.awt.Component c, boolean recursive) {   // Make sure we support dnd.
        if (supportsDnD()) {
        	Hotels.log("FileDrop: Removing drag-and-drop hooks.");
            c.setDropTarget(null);
            if (recursive && (c instanceof java.awt.Container)) {
